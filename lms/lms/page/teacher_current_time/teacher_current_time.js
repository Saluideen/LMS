frappe.pages['teacher-current-time'].on_page_load = function(wrapper) {
...
day=currentDayName
	...
	frappe.call({
	   ...
	   callback: function (r) {
		...
		console.log("teacher",r.message[0].name);
		   teacher=r.message[0].name
	...
	frappe.call({
		...
		callback: function (r) {
			...
			if (r.message.length > 0) {
				r.message.forEach(el => {
					hours.push([el.hour, el.subject, el.Standard, el.division, el.from_time, el.to_time]);
		...
		let hour,standard,division,n_subject
		console.log("day",day,teacher);
		frappe.call({
	...
	var tableStructure = [
		...
		['Hour', 'Subject', 'Standard', 'Division', 'From', 'To'],
